# ============================================
# Socartes Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for Socartes
...
# Labels
LABEL maintainer="Socartes Team" \
      description="Socartes: AI-Powered Personalized Learning Assistant" \
      version="1.0.0"
...
COPY socartes/ ./socartes/
COPY socartes_cli/ ./socartes_cli/
...
RUN cat > /etc/supervisor/conf.d/socartes.conf <<'EOF'
[supervisord]
nodaemon=true
logfile=/dev/null
logfile_maxbytes=0
...
RUN sed -i 's/\r$//' /etc/supervisor/conf.d/socartes.conf
...
exec python -m uvicorn socartes.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT}
EOF
...
# Start supervisord
exec /usr/bin/supervisord -c /etc/supervisor/conf.d/socartes.conf
EOF
...
[program:backend]
command=python -m uvicorn socartes.api.main:app --host 0.0.0.0 --port %(ENV_BACKEND_PORT)s --reload
directory=/app
autostart=true
autorestart=true
stdout_logfile=/dev/fd/1
