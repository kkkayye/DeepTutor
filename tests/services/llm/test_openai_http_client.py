...
from socartes.services.llm import openai_http_client
...
def _clean_ssl_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    clear_cache = getattr(openai_http_client, "clear_cached_http_clients", None)
...
def test_provider_core_passes_disable_ssl_http_client(monkeypatch: pytest.MonkeyPatch) -> None:
    from socartes.services.llm.provider_core import openai_compat_provider as provider_mod
...
def test_azure_provider_passes_disable_ssl_http_client(monkeypatch: pytest.MonkeyPatch) -> None:
    from socartes.services.llm.provider_core import azure_openai_provider as azure_mod
...
def test_tutorbot_provider_passes_disable_ssl_http_client(
    ...
    from socartes.tutorbot.providers import openai_compat_provider as tutorbot_mod
...
async def test_sdk_complete_passes_disable_ssl_http_client(
    ...
    from socartes.services.llm import executors
...
@pytest.mark.asyncio
async def test_sdk_complete_reuses_async_openai_client(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from socartes.services.llm import executors
...
def test_embedding_sdk_passes_disable_ssl_http_client(
    ...
    from socartes.services.embedding.adapters import openai_sdk as embedding_mod
