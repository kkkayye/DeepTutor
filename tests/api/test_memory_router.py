...
router = importlib.import_module("socartes.api.routers.memory").router
...
def test_memory_router_returns_single_document(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "socartes.api.routers.memory.get_memory_service", lambda: FakeMemoryService()
    )
...
def test_memory_router_refreshes_from_session(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "socartes.api.routers.memory.get_sqlite_session_store", lambda: FakeStore()
    )
    monkeypatch.setattr(
        "socartes.api.routers.memory.get_memory_service", lambda: FakeMemoryService()
    )
