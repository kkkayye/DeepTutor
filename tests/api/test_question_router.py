...
def _cleanup_question_router_module():
    yield
    sys.modules.pop("socartes.api.routers.question", None)
...
def _load_question_router_module(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_agents = _package("socartes.agents")
    fake_agents_question = types.ModuleType("socartes.agents.question")
    fake_agents_question.AgentCoordinator = object
    fake_agents.question = fake_agents_question
    monkeypatch.setitem(sys.modules, "socartes.agents", fake_agents)
    monkeypatch.setitem(sys.modules, "socartes.agents.question", fake_agents_question)

    fake_logging = _package("socartes.logging")
    ...
    monkeypatch.setitem(sys.modules, "socartes.logging", fake_logging)

    fake_config = types.ModuleType("socartes.services.config")
    fake_config.PROJECT_ROOT = Path.cwd()
    fake_config.load_config_with_main = lambda *_args, **_kwargs: {}
    monkeypatch.setitem(sys.modules, "socartes.services.config", fake_config)

    fake_llm_package = _package("socartes.services.llm")
    fake_llm_config = types.ModuleType("socartes.services.llm.config")
    fake_llm_config.get_llm_config = lambda: None
    fake_llm_package.config = fake_llm_config
    monkeypatch.setitem(sys.modules, "socartes.services.llm", fake_llm_package)
    monkeypatch.setitem(sys.modules, "socartes.services.llm.config", fake_llm_config)

    fake_settings_package = _package("socartes.services.settings")
    fake_interface_settings = types.ModuleType("socartes.services.settings.interface_settings")
    ...
    monkeypatch.setitem(sys.modules, "socartes.services.settings", fake_settings_package)
    monkeypatch.setitem(
        sys.modules,
        "socartes.services.settings.interface_settings",
        fake_interface_settings,
    )

    fake_tools = _package("socartes.tools")
    fake_tools_question = types.ModuleType("socartes.tools.question")
    ...
    monkeypatch.setitem(sys.modules, "socartes.tools", fake_tools)
    monkeypatch.setitem(sys.modules, "socartes.tools.question", fake_tools_question)

    return importlib.import_module("socartes.api.routers.question")
