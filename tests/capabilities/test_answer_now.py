...
``socartes.capabilities._answer_now.llm_stream`` (the alias the helper
imports) and ``socartes.capabilities._answer_now.get_llm_config`` so
...
from socartes.capabilities import _answer_now
from socartes.capabilities._answer_now import (
...
class TestChatAnswerNow:
    ...
    async def test_chat_synthesises_final_answer_from_partial_trace(self) -> None:
        from socartes.agents.chat import agentic_pipeline as ap_module
...
class TestMathAnimatorAnswerNow:
    ...
    async def test_skips_analysis_design_summary_but_calls_codegen_and_render(
        ...
        from socartes.agents.math_animator.models import (
            GeneratedCode,
            RenderResult,
        )
        ...
        from socartes.capabilities.math_animator import MathAnimatorCapability

        analysis_agent_calls: list[Any] = []
        design_agent_calls: list[Any] = []
        summary_agent_calls: list[Any] = []

        with patch("socartes.agents.math_animator.pipeline.MathAnimatorPipeline") as PipelineCls:
...
class TestNormalPathWhenNoAnswerNow:
    ...
    async def test_deep_solve_without_payload_calls_main_solver(self) -> None:
        ...
        with (
            patch("socartes.agents.solve.main_solver.MainSolver") as SolverCls,
            patch("socartes.services.llm.config.get_llm_config", return_value=_fake_llm_config()),
        ):
