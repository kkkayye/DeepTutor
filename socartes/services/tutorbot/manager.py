...
Each TutorBot instance runs as a set of asyncio tasks within the Socartes
...
class TutorBotManager:
    ...
    def _shared_memory_dir(self) -> Path:
        """Public memory shared by Socartes and all bots."""
    ...
    async def start_bot(self, bot_id: str, config: BotConfig | None = None) -> TutorBotInstance:
        ...
        from socartes.services.tutorbot.model_runtime import (
        ...
        from socartes.tutorbot.config.schema import ExecToolConfig
        from socartes.tutorbot.providers.socartes_adapter import create_socartes_provider
        from socartes.tutorbot.session.manager import SessionManager

        llm_config = resolve_tutorbot_llm_config(config)
        provider = create_socartes_provider(llm_config)
        bus = MessageBus()
        ...
        # -- Heartbeat --------------------------------------------------------
        from socartes.tutorbot.heartbeat import HeartbeatService
    ...
    async def _outbound_router(self, bot_id: str, bus: Any, instance: TutorBotInstance) -> None:
        ...
        """
        try:
            from socartes.events.event_bus import Event, EventType, get_event_bus
            from socartes.tutorbot.bus.events import OutboundMessage as _OMsg
    ...
    def _build_channel_manager(
        ...
        *,
        ...
        from socartes.tutorbot.config.schema import ChannelsConfig
