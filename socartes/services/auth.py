"""
Authentication service for Socartes.
...
TOKEN_EXPIRE_HOURS: int = int(os.getenv("AUTH_TOKEN_EXPIRE_HOURS") or "24")
...
if AUTH_ENABLED and not POCKETBASE_ENABLED and not AUTH_SECRET:
    from socartes.multi_user.identity import load_or_create_auth_secret
...
def _make_user_record(hashed: str, role: str = "user", created_at: str = "") -> dict[str, Any]:
    ...
    from socartes.multi_user.identity import new_user_id
...
def _load_users() -> dict[str, dict]:
    ...
    """
    from socartes.multi_user.identity import load_users
...
def list_users() -> list[dict]:
    ...
    from socartes.multi_user.identity import list_user_info
...
def delete_user(username: str) -> bool:
    ...
    """
    from socartes.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from socartes.multi_user.identity import set_role as _set_role
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from socartes.services.pocketbase_client import validate_pb_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@socartes.local to match the email used at registration.
    """
    try:
