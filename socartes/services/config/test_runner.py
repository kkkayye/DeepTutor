...
from socartes.services.llm.client import reset_llm_client
from socartes.services.llm.config import clear_llm_config_cache
...
class ConfigTestRunner:
    ...
    def _persist_embedding_dimension(
        ...
        use the new dim.
        """
        from socartes.services.embedding.client import reset_embedding_client
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from socartes.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from socartes.services.llm import complete as llm_complete
    ...
    async def _test_embedding(
        ...
        from socartes.services.embedding.client import EmbeddingClient
        from socartes.services.embedding.config import EmbeddingConfig
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from socartes.services.search import web_search
        ...
        run.emit("info", "Running search query: Socartes configuration health check")
        result = web_search("Socartes configuration health check", provider=provider)
        run.emit(
            "response",
