...
from socartes.services.session.protocol import SessionStoreProtocol
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from socartes.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from socartes.services.session import get_session_store
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from socartes.capabilities.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from socartes.multi_user.model_access import apply_allowed_llm_selection
            ...
            from socartes.multi_user.model_access import redacted_model_access
        ...
        if llm_selection:
            from socartes.services.config import get_model_catalog_service
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from socartes.agents.notebook import NotebookAnalysisAgent
            from socartes.book.context import build_book_context
            from socartes.core.context import Attachment, UnifiedContext
            from socartes.runtime.orchestrator import ChatOrchestrator
            from socartes.services.memory import get_memory_service
            from socartes.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from socartes.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from socartes.services.notebook import get_notebook_manager
            from socartes.services.session.context_builder import ContextBuilder
            from socartes.services.skill import get_skill_service
            ...
            from socartes.services.storage import get_attachment_store
            ...
            from socartes.utils.document_extractor import extract_documents_from_records
            ...
            from socartes.multi_user.context import get_current_user
            from socartes.multi_user.paths import get_admin_path_service
            from socartes.multi_user.skill_access import assigned_skill_ids
