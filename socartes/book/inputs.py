...
Reuses:
- ``socartes.services.session.get_sqlite_session_store`` for chat history
- ``socartes.services.notebook.notebook_manager`` for notebook records
- ``socartes.agents.notebook.NotebookAnalysisAgent`` for context distillation
"""
...
async def _resolve_chat_selections(
    ...
    *,
    ...
    if not selections:
        return []
    try:
        from socartes.services.session import get_sqlite_session_store
...
async def _resolve_notebook_context(
    ...
    *,
    ...
    if not notebook_refs:
        return "", 0
    try:
        from socartes.services.notebook import notebook_manager
