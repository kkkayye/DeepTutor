"""SessionManager adapter backed by Socartes's SQLite store.

Implements the SessionManager interface (get_or_create, save, list_sessions) but
reads/writes through Socartes's SQLiteSessionStore, unifying conversation history
for TutorBot and Socartes in a single database.
"""
...
from socartes.tutorbot.session.manager import Session


class SQLiteSessionAdapter:
    """Drop-in replacement for SessionManager, backed by Socartes SQLite."""

    def __init__(self, store: Any) -> None:
        """
        Args:
            store: A Socartes SQLiteSessionStore instance.
        """
        self.store = store
    ...
    def _session_id(self, key: str) -> str:
        """Derive a stable Socartes session_id from a TutorBot key (channel:chat_id)."""
    ...
    def _ensure_sqlite_session_sync(self, key: str) -> None:
        """Ensure a corresponding Socartes session row exists."""
        session_id = self._session_id(key)
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
