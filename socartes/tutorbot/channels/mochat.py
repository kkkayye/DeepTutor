...
from socartes.tutorbot.config.paths import get_runtime_subdir
from socartes.tutorbot.config.schema import Base
    ...
    async def stop(self) -> None:
        ...
        if self._socket:
            try:
                ...
                logger.opt(exception=True).debug("Mochat websocket disconnect failed")
            self._socket = None
    ...
    async def _start_socket_client(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.opt(exception=True).debug("Mochat websocket cleanup disconnect failed")
            self._socket = None
            return False
