...
async def unified_websocket(ws: WebSocket) -> None:
    ...
    from socartes.multi_user.context import (
        reset_current_user,
        set_current_user,
        user_from_token_payload,
    )
    from socartes.multi_user.paths import local_admin_user
    from socartes.services.auth import AUTH_ENABLED, decode_token
    ...
    async def subscribe_turn(turn_id: str, after_seq: int = 0) -> None:
        from socartes.services.session import get_turn_runtime_manager
