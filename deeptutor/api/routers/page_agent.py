...
def get_llm_config():
    from deeptutor.services.llm.config import get_llm_config as load_llm_config
...
def _fallback_completion(model: str = "deeptutor-page-agent-fallback") -> dict[str, Any]:
    arguments = {
        "type": "done",
        "message": (
...
async def create_chat_completion(request: ChatCompletionRequest) -> dict[str, Any]:
    try:
        ...
        return _fallback_completion(request.model or "deeptutor-page-agent-fallback")
